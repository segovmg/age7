...
exit_code <- readLines(exit_code_path)
...
O fluxo ETL de atualização do projeto age7 foi concluído com `r ifelse(exit_code == 0, "SUCESSO", "__FRACASSO__")` em `r file.mtime(exit_code_path)`.

Os logs da execução estão disponíveis no commit `r glue::glue('[{commit}](https://github.com/transparencia-mg/age7/commit/{commit})')`.
